    ...
    public int myAtoi(String s) {
		...
		for(int i = 0; i < s.length() ; i++)
		{
			if(Character.isSpace(s.charAt(i))){
				...
				{
					if((s.charAt(i+j) >= '0') && (s.charAt(i+j) <= '9')){

						if ((sign == 1) && (result > Integer.MAX_VALUE/10)){
							result = Integer.MAX_VALUE;
							return result;
						}
						else if ((sign == -1) && (result > Integer.MAX_VALUE/10)){
							result = Integer.MIN_VALUE;
							return result;
						}
						else if ((sign == 1) && (result <= Integer.MAX_VALUE/10)){
							if( s.charAt(i+j)- '0' <= Integer.MAX_VALUE - 10 * result)
							{
								result = 10 * result + s.charAt(i+j) - '0';
							}
							else
							{
								result = Integer.MAX_VALUE;
